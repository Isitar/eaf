...
public class Test1 implements CommandLineRunner {
    ...
    private void test1() {
        ...
        TypedQuery<User> query = em.createQuery(
                "SELECT u from User u " + "join u.rentals r " + "where r.movie.title = :movie", User.class);
        // andere möglichkeit
        query = em.createQuery("SELECT r.User from Rental r where r.movie.title = :movie", User.class);
